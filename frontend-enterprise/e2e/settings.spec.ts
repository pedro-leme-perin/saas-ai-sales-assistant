...
test.describe('Settings Page (authenticated)', () => {
  ...
  test('should have default tab (Profile) active on initial load', async ({ page }) => {
    await page.goto('/dashboard/settings');
  ...
  test('should navigate to Company tab', async ({ page }) => {
    ...
    const companyContent = page.locator('text=/empresa|company name|razão social/i');
  ...
  test('should navigate to Notifications tab', async ({ page }) => {
    ...
    const notificationsTab = page.getByRole('button', { name: /notificações|notifications/i });
    ...
    const notificationsContent = page.locator('text=/notificações|notifications|email|alerts/i');
  ...
  test('should navigate to Security tab', async ({ page }) => {
    ...
    // Click Security tab
    const securityTab = page.getByRole('button', { name: /segurança|security/i });
    ...
    const securityContent = page.locator('text=/segurança|security|senha|password|two-factor/i');
  ...
  test('should navigate to Appearance tab', async ({ page }) => {
    ...
    const appearanceTab = page.getByRole('button', { name: /aparência|appearance/i });
    ...
    const appearanceContent = page.locator('text=/aparência|appearance|tema|theme|idioma|language/i');
  ...
  test('should have language selector in Appearance tab', async ({ page }) => {
    ...
    const languageSelector = page.locator('text=/idioma|language|português|english|pt|en/i');

    // Language selector should be visible
    expect(await languageSelector.first().isVisible()).toBeTruthy();

    // Check for language options
    const languageOptions = page.locator('button, select').filter({ hasText: /português|english|pt-br|en/i });
  ...
  test('should have dark mode toggle in Appearance tab', async ({ page }) => {
    ...
    const themeToggle = page.getByRole('button', { name: /tema|theme|claro|escuro|light|dark|modo noturno/i });
  ...
  test('should load all tabs without JavaScript errors', async ({ page }) => {
    ...
    for (const tab of [profileTab, companyTab, notificationsTab, securityTab, appearanceTab]) {
      await tab.click();
      await page.waitForLoadState('networkidle');
    }

    // Should have minimal or no JS errors
    expect(errors.filter(e => !e.includes('Network.getResponseBody'))).toHaveLength(0);
  });

  test('should render settings sidebar with all tabs visible', async ({ page }) => {
    await page.goto('/dashboard/settings');
    await page.waitForLoadState('networkidle');
  ...
  test('should show skeleton loaders while tab content loads', async ({ page }) => {
