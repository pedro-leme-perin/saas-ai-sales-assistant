...
$LogFile = "scripts\s74-commit.log"
"=== S74 commit start: $(Get-Date) ===" | Tee-Object -FilePath $LogFile
...
Write-Host ""
Write-Host "[2/5] pnpm install (refresh lockfile com 3 novos overrides)..."
...
Write-Host ""
Write-Host "[3/5] git add (5 source files + lockfile)..."
git add package.json pnpm-lock.yaml .github/workflows/ci.yml CHANGELOG.md PROJECT_HISTORY.md CLAUDE.md scripts/s74-commit.ps1 scripts/s74-commit.bat scripts/s74-commit-msg.txt 2>&1 | Tee-Object -FilePath $LogFile -Append

Write-Host ""
Write-Host "[4/5] git commit (HUSKY=1, hooks vao rodar pre-commit + commit-msg)..."
git commit -F scripts\s74-commit-msg.txt 2>&1 | Tee-Object -FilePath $LogFile -Append
if ($LASTEXITCODE -ne 0) {
...
"=== S74 commit done: $(Get-Date) ===" | Tee-Object -FilePath $LogFile -Append
Write-Host ""
