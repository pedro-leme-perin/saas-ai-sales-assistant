...
| Último commit | S60a-deploy (30/04/2026) | **Feature S60a — DSAR workflow LGPD Art. 18.** Novo módulo `dsar` (controller + service + extract worker), schema `DsarRequest` (42 modelos total), 2 enums novos (`DsarType`, `DsarStatus`), 4 valores `AuditAction` (DSAR_REQUESTED/APPROVED/REJECTED/COMPLETED), 1 valor `BackgroundJobType` (EXTRACT_DSAR), 1 valor `RetentionResource` (DSAR_ARTIFACTS). 5 tipos suportados: ACCESS, PORTABILITY, CORRECTION, DELETION, INFO. DELETION reusa `LgpdDeletionService.scheduleDeletionForDsar` (grace 30d para User match) + soft-delete Contact + anonymize CsatResponse (sem grace, dado não-conta). Artefato JSON server-generated → `UploadService.putObject` (R2 V4 SHA-256 signed PUT) → presigned GET URL 7d expiry. Frontend `/dashboard/admin/dsar`. State machine: PENDING→APPROVED/REJECTED, APPROVED→PROCESSING, PROCESSING→COMPLETED/FAILED, COMPLETED→EXPIRED. Cron `dsar-expiry-tick @EVERY_HOUR`. Anti-abuse: cap 3 requests abertas por requester em 7d. Tests: `dsar.service.spec.ts` + `dsar-extract.service.spec.ts` (state machine, RBAC, type-branching, idempotency, failure path). Anterior: S59-hotfix `c3f44a9` CI verde 3m 16s. |
