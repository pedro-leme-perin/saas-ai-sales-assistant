...
test.describe("WhatsApp Page (authenticated)", () => {
  ...
  test("should render whatsapp page with conversations title", async ({
    page,
  }) => {
  ...
  test("should have searchable chat input for phone numbers", async ({
    page,
  }) => {
  ...
  test("should render message area layout when chat selected", async ({
    page,
  }) => {
    ...
    const messageArea = page
      .locator("text=/selecione uma conversa|select a conversation/i")
      .first();
  ...
  test("should support dark mode compatibility", async ({ page }) => {
    ...
    await expect(
      page.locator("text=/conversas|conversations/i").first(),
    ).toBeVisible();
  ...
  test("should render AI suggestion buttons when available", async ({
    page,
  }) => {
    // The AI suggestion button with Sparkles icon should exist
    const aiButton = page.getByRole("button", {
      name: /ask ai|pedir ia|sparkles/i,
    });
    ...
    const hasPageStructure =
      (await page.locator('[class*="flex"]').count()) > 0;
    expect(hasPageStructure).toBe(true);
  });

  test("should render skeleton loaders during chat loading", async ({
    page,
  }) => {
