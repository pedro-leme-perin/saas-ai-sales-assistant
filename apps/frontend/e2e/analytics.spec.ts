...
test.describe('Analytics Page (authenticated)', () => {
  ...
  test('should display KPI cards visible', async ({ page }) => {
    ...
    const hasKPIContent = await page.locator('text=/calls|ligações|messages|mensagens|users|usuários/i').count();
  ...
  test('should display calls detail card', async ({ page }) => {
    ...
    const metricsVisible = await page.locator('text=/total|completed|sucesso|success rate/i').count();
  ...
  test('should display whatsapp detail card', async ({ page }) => {
    ...
    const waMetrics = await page.locator('text=/chats|mensagens|messages|open/i').count();
  ...
  test('should display AI performance card with metrics', async ({ page }) => {
    ...
    const aiMetrics = await page.locator('text=/suggestions|sugestões|adoption|adoptadas/i').count();
    expect(aiMetrics >= 0).toBe(true);
  });

  test('should load sentiment analytics section dynamically', async ({ page }) => {
  ...
  test('should load AI performance detail section dynamically', async ({ page }) => {
    ...
    const hasDetailSections = await page.locator('[class*="md:grid-cols-2"]').count();
  ...
  test('should display KPI grid layout with responsive columns', async ({ page }) => {
    await page.waitForSelector('h1');
  ...
  test('should load without JavaScript errors', async ({ page }) => {
    ...
    await expect(page.locator('h1').first()).toContainText(/analytics|analytiques/i);
