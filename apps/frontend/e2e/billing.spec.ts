...
test.describe("Billing Page (authenticated)", () => {
  ...
  test("should display current plan card", async ({ page }) => {
    ...
    const subscriptionHeader = page
      .locator("h2")
      .filter({ hasText: /assinatura atual|current subscription/i });
  ...
  test("should display plan cards (Starter, Professional, Enterprise)", async ({
    page,
  }) => {
    await page.goto("/dashboard/billing");
    ...
    const plansHeader = page
      .locator("h2")
      .filter({ hasText: /planos disponíveis|available plans/i });
  ...
  test("should display invoices section", async ({ page }) => {
    ...
    const invoicesHeader = page
      .locator("h2")
      .filter({ hasText: /histórico de faturas|invoice history/i });
  ...
  test("should have upgrade button for non-current plans", async ({ page }) => {
    ...
    const upgradeButton = page.getByRole("button", {
      name: /upgrade|selecionar|select|mudar para|change to/i,
    });
