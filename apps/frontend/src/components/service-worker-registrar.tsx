...
export function ServiceWorkerRegistrar() {
  ...
  useEffect(() => {
    ...
    registerServiceWorker({
      onUpdate: () => {
        toast.info(t("notifications.sw_update_title") || "Update available", {
          description:
    ...
    const unsubscribe = onOnlineStatusChange((isOnline) => {
      if (!isOnline) {
        toast.error(t("notifications.offline_title") || "You are offline", {
          description:
            t("notifications.offline_description") ||
            "Some features may be unavailable. Changes will sync when reconnected.",
        });
      } else {
        toast.success(t("notifications.online_title") || "Back online", {
          description:
