  ...
  return (
    <div className="space-y-4 p-4">
      {[...Array(5)].map((_, i) => (
        ...
        >
          <div
            className={`rounded-2xl animate-pulse bg-muted ${i % 2 === 0 ? "w-48 h-12" : "w-56 h-16"}`}
          />
        </div>
      ))}
    </div>
...
export default function WhatsAppPage() {
  ...
  const { setActiveChat, otherUserTyping, setOtherUserTyping } =
    useActiveChatStore();
  const { currentSuggestion, setCurrentSuggestion, setGenerating } =
    useAISuggestionsStore();
  ...
  const { data: chatsData, isLoading: chatsLoading } = useQuery({
    ...
    queryFn: () =>
      whatsappService.getChats({ search: searchQuery || undefined }),
  });
  ...
  const sendMessageMutation = useMutation({
    ...
    onError: (error: any) => {
      toast.error(t("whatsapp.errorSendMessage"), {
        ...
        <Card className="flex-1 flex flex-col">
          ...
          <div className="flex items-center gap-3 p-4 border-b">
            ...
            <Button
              variant="ghost"
              size="icon"
              aria-label={t("whatsapp.callLabel")}
            >
            ...
            <Button
              variant="ghost"
              size="icon"
              aria-label={t("whatsapp.moreOptions")}
            >
                    ...
                    {currentSuggestion.type && (
                      <span
                        className={`text-[10px] px-1.5 py-0.5 rounded-full ${
                          SUGGESTION_TYPE_COLORS[currentSuggestion.type] ||
                          "bg-muted"
                        }`}
                      >
          ...
          <div className="p-4 border-t">
            <div className="flex items-center gap-2">
              <Button
                variant="ghost"
                size="icon"
                aria-label={t("whatsapp.emojisLabel")}
                className="shrink-0"
              >
              ...
              <Button
                variant="ghost"
                size="icon"
                aria-label={t("whatsapp.attachFile")}
                className="shrink-0"
              >
        ...
        <Card className="hidden md:flex flex-1 items-center justify-center">
          <div className="text-center">
            ...
            <h3 className="text-lg font-medium mb-2">
              {t("whatsapp.selectChat")}
            </h3>
            <p className="text-sm text-muted-foreground">
              {t("whatsapp.selectChatHint")}
            </p>
          </div>
        </Card>
      )}
