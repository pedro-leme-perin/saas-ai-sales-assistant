...
export type UpdateScheduledExportInput = Partial<Omit<CreateScheduledExportInput, 'resource'>>;

export const scheduledExportsService = {
  list: async () => {
    return apiClient.get<ScheduledExport[]>(`/scheduled-exports`);
  },
  ...
  remove: (id: string) => apiClient.delete<{ success: true }>(`/scheduled-exports/${id}`),
  runNow: (id: string) => apiClient.post<ScheduledExport>(`/scheduled-exports/${id}/run-now`, {}),
};
