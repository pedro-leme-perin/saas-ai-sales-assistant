...
export const scheduledMessagesService = {
  schedule: (chatId: string, input: CreateScheduledMessageInput) =>
    apiClient.post<ScheduledMessage>(`/whatsapp/chats/${chatId}/schedule`, input),
  list: async (filters: ListScheduledMessagesFilters = {}) => {
    ...
    return apiClient.get<ScheduledMessage[]>(`/scheduled-messages${qs ? `?${qs}` : ''}`);
  },
  findById: (id: string) => apiClient.get<ScheduledMessage>(`/scheduled-messages/${id}`),
  cancel: (id: string) => apiClient.delete<ScheduledMessage>(`/scheduled-messages/${id}`),
};
