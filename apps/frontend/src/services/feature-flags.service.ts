...
export const featureFlagsService = {
  list: async () => {
    return apiClient.get<FeatureFlag[]>(`/feature-flags`);
  },
  findById: (id: string) => apiClient.get<FeatureFlag>(`/feature-flags/${id}`),
  create: (dto: CreateFeatureFlagInput) => apiClient.post<FeatureFlag>(`/feature-flags`, dto),
  update: (id: string, dto: UpdateFeatureFlagInput) =>
    apiClient.patch<FeatureFlag>(`/feature-flags/${id}`, dto),
  remove: (id: string) => apiClient.delete<{ success: true }>(`/feature-flags/${id}`),
