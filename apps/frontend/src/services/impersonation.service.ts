...
async function start(payload: StartImpersonationPayload): Promise<StartImpersonationResult> {
  return apiClient.post<StartImpersonationResult>('/impersonation/start', payload);
}

async function end(sessionId: string, reason?: string): Promise<{ ended: boolean }> {
...
async function findById(sessionId: string): Promise<ImpersonationSessionDetail> {
  return apiClient.get<ImpersonationSessionDetail>(`/impersonation/sessions/${sessionId}`);
}
