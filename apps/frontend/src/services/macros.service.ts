...
export type MacroActionType = 'SEND_REPLY' | 'ATTACH_TAG' | 'ASSIGN_AGENT' | 'CLOSE_CHAT';
...
export type MacroAction = SendReplyAction | AttachTagAction | AssignAgentAction | CloseChatAction;
...
export const macrosService = {
  list: async () => {
    return apiClient.get<Macro[]>(`/macros`);
  },
  findById: (id: string) => apiClient.get<Macro>(`/macros/${id}`),
  create: (dto: CreateMacroInput) => apiClient.post<Macro>(`/macros`, dto),
  update: (id: string, dto: UpdateMacroInput) => apiClient.patch<Macro>(`/macros/${id}`, dto),
  remove: (id: string) => apiClient.delete<{ success: true }>(`/macros/${id}`),
